...
fn replace_uri_username_password(v: &str) -> Cow<'_, str> {
  ...
  let r = RE.get_or_init(|| Regex::new(r#"://(.+):(.+)@"#).unwrap());
...
mod tests {
  ...
  #[test]
  fn test_url_string() {
    let url_string = UriString::new("postgres://abcdefg:2023.fusion@localhost:5432/fusion");
    let text = serde_json::to_string(&url_string).unwrap();
    assert_eq!(text, "\"postgres://<username>:<password>@localhost:5432/fusion\"");

    let url_string = UriString::new("postgres://localhost:5432/fusion");
    let text = serde_json::to_string(&url_string).unwrap();
    assert_eq!(text, "\"postgres://localhost:5432/fusion\"");
  }
}
