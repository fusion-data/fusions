use crate::utils::{get_input_attribute, get_meta_value_string};
...
pub fn get_struct_fusionsql_props(dinput: &DeriveInput) -> Result<StructModqlFieldProps, syn::Error> {
  // FIXME: We should remove this, 'sqlb' should not be a thing anymore.
  let sqlb_attr = get_input_attribute(dinput, "fusionsql");
  let mut rel = None;
  ...
  if let Some(attribute) = sqlb_attr {
    ...
    for meta in nested {
      match meta {
        ...
        Meta::Path(path) => {
          if let Some(path) = path.get_ident() {
            ...
            if path == "names_as_consts" {
              names_as_consts = Some(String::new());
            }
          }
        }
        ...
        /* ... */
        Meta::List(_) => {
          let msg = "unrecognized fusionsql attribute value";
