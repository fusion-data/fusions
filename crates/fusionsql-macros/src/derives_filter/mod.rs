...
use crate::utils::struct_fusionsql_attr::get_struct_fusionsql_props;
  ...
  for field in &fields.named {
    ...
    if type_name.starts_with("Option ") && type_name.contains("OpVal") {
      if let Some(ident) = field.ident.as_ref() {
        ...
        let quote_filter_node_options_cast_as = if let Some(cast_as) = fusionsql_field_attr.cast_as {
        ...
        let block = if let Some(to_sea_condition_fn) = fusionsql_field_attr.to_sea_condition_fn {
