...
pub(crate) fn derive_field_sea_value_inner(input: TokenStream) -> TokenStream {
  ...
  let expanded = match input.data {
    syn::Data::Enum(ref data) => process_enum(&name, data),
    syn::Data::Struct(ref data) => match process_struct(&name, data) {
      Ok(ts) => ts,
      // proc-macro 内不再 panic：返回 compile_error! 让下游看到出错 span。
      Err(err) => err.to_compile_error(),
    },
    syn::Data::Union(ref data) => {
      syn::Error::new_spanned(data.union_token, "SeaFieldValue can only be used with enums and tuple structs")
        .to_compile_error()
    }
  };
...
fn process_struct(name: &Ident, data: &DataStruct) -> syn::Result<proc_macro2::TokenStream> {
  let first_tuple_field = match &data.fields {
    Fields::Unnamed(fields) if fields.unnamed.len() == 1 => fields.unnamed.iter().next().unwrap(),
    _ => {
      return Err(syn::Error::new_spanned(&data.fields, "expected a tuple struct with exactly one field"));
    }
  };
  ...
  let field_type_path = match field_type {
    Type::Group(group) => match &*group.elem {
      Type::Path(p) => p.clone(),
      other => {
        return Err(syn::Error::new_spanned(other, "unsupported type: TypeGroup element is not a path"));
      }
    },
    Type::Path(p) => p.clone(),
    other => {
      return Err(syn::Error::new_spanned(other, "unsupported type: expected a path type"));
    }
  };
  ...
  let value_variant = match field_type_path.path.get_ident() {
    Some(ident) => match ident.to_string().as_str() {
      ...
      _ => {
        return Err(syn::Error::new_spanned(ident, format!("unsupported field type: {ident}")));
      }
    },
    None => {
      return Err(syn::Error::new_spanned(&field_type_path, "unsupported type: no type identifier found"));
    }
  };
