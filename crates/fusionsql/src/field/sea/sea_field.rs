use fusionsql_core::sea_utils::{SIden, StringIden};
...
impl SeaField {
    ...
    T: ValueType,
  {
    ...
    T::try_from(value).map_err(|_| Error::FieldValueIntoTypeError { field_name: self.iden.to_string() })
  }
}
...
impl SeaField {
  ...
  pub fn new_concrete(iden: DynIden, value: SimpleExpr) -> Self {
    ...
    SeaField { iden, column_ref, value }
  }
  ...
  pub fn siden(iden: &'static str, value: impl Into<SimpleExpr>) -> Self {
    ...
    let column_ref = ColumnRef::Column(iden.clone());
    SeaField { iden, column_ref, value: value.into() }
  }
