...
impl Context {
  ...
  /// # Panics
  ///
  /// Panics if `value` cannot be serialized to JSON (e.g. a type whose
  /// `Serialize` implementation returns an error, such as a map with
  /// non-string keys or a value containing a non-finite float in a
  /// context that rejects it).
  ...
  pub async fn add_user_message(&self, content: String) {
    let mut history = self.chat_history.write().unwrap_or_else(|e| e.into_inner());
  ...
  pub async fn get_chat_history(&self) -> ChatHistory {
    self.chat_history.read().unwrap_or_else(|e| e.into_inner()).clone()
  }
  ...
  pub async fn chat_history_len(&self) -> usize {
    self.chat_history.read().unwrap_or_else(|e| e.into_inner()).len()
  }

  /// Check if the chat history is empty.
  pub async fn is_chat_history_empty(&self) -> bool {
    self.chat_history.read().unwrap_or_else(|e| e.into_inner()).is_empty()
  }
