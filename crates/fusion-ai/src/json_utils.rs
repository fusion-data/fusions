...
///
/// Recursive (shallow key-wise) merge is only performed when **both** sides
/// are JSON objects; for any other combination the left value `a` is returned
/// unchanged and `b` is discarded.
...
mod tests {
  ...
  fn test_stringified_json_serialize() {
    let dummy = Dummy { data: serde_json::json!({"key": "value"}) };
  ...
  fn test_stringified_json_deserialize() {
    ...
    let expected = Dummy { data: serde_json::json!({"key": "value"}) };
