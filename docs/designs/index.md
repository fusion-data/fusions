...
| [speech-providers](./speech-providers.md) | 框架设计 | 语音协议客户端统一抽象：`providers::speech` 底座 + MiniMax / 豆包 / 阿里三家 TTS **比价 SSOT（§2 协议 + 价格 + 能力对比表）**、分层切分判据、计费语义坑、校准点 |
| [storage](./storage.md) | 框架规格 | `fusion-storage` 对象存储机制层：feature 透传面（opendal services-* 归属）、`build_operator` 错误三分级、presign capability gate 分流与 wire 契约锁定、签名缓存与桶对齐边界 |
