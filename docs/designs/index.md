...
| 文档 | 类型 | 摘要 |
|---|---|---|
| [framework-conventions](./framework-conventions.md) | 框架约定 SSOT | `fusion-*` / `fusionsql*` 横切约定：错误语义、安全默认（携密 Debug 脱敏 / ORDER BY 实体列名单）、并发资源（client 复用 / abort-on-drop）、事务感知路由、DI API 命名（panic/`try_` 配对）、宏路径卫生 |
