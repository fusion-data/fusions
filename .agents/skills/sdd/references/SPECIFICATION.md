...
Level 4 判定（§13.1）中，部分质量属性无法并入常规 UAT 章节——本节定义何时立质量专项、专项产出文档的最小结构与归档保留判据。项目载体（证据目录路径、环境档位取值）由项目 overlay / UAT 文档登记，本节不复述。

- **立项判据**（满足其一才立专项；否则并入常规 UAT，MUST NOT 为常规场景另立专项）：
  1. 判定空间是「环境 × 维度 × 场景」的组合（如导出产物在多个办公套件中的可编辑性），单条 UAT 场景承载不下；
  2. 需要「对照实验」隔离单一变量的净效果（如多轮输入仅差一个注入段）。
- **产出文档最小结构**（矩阵型与对照实验型共用；「判定空间」一项按型取形）：
  - 质量红线 / 判定问题：一句话判据（如「每个文本框可正常再编辑」）或对照要回答的问题；
  - 环境定档表：每环境标注必备 / 采样 / 观察 / 取消 + 定档依据；定档与取消均属 Human Approval Gate，MUST 留痕决策者与理由（预授权自动决策时留痕授权来源）；对照实验型无环境维度时 MAY 省略本表；
  - 判定空间：矩阵型 = 组合定义 + 样张清单与代码 fixture 一一对应（代码为真相源，增删样张 MUST 双向同步）；对照实验型 = 轮次设计与唯一变量声明（各轮除被隔离变量外输入一致）+ 采集方式可复跑（脚本随证据落档）；
  - AUTO 证据指针与人工签收状态 MUST 分列（口径即 §13.3，不另立）；
  - 人工判定项（Human Approval Gate）：证据供人工复核，复核结论就地更新；
  - 维护协议：判定空间或 fixture 变更时文档的跟进义务。
- **归档保留与回流**：专项计划按 §4.6 归档删除时，被现行测试或 UAT 覆盖矩阵引用的矩阵文档是常驻真相源，MUST NOT 随计划删除；一次性采集的证据（实机操作记录 / 对照轮次记录）保留备查。专项签收后 MUST 在常规 UAT 覆盖矩阵登记指针（AUTO 证据指针 + 签收结论），MUST NOT 游离于 UAT 体系之外。

> 本条是「质量专项验收」（立项判据 / 产出结构 / 归档保留与回流）的概念真相源；证据目录路径等项目取值由项目 overlay 登记。
