...
//!
//! In order to run the example call, edit .env:
//!
//! ```sh
//! cargo run -p fusion-security --features="with-oauth" --example github-async
//! ```
//!
...
use fusions::security::oauth2::basic::BasicClient;
use fusions::security::oauth2::reqwest;
use fusions::security::oauth2::{
...
async fn main() {
  ...
  let github_client_id =
    ClientId::new(env::var("GITHUB_CLIENT_ID").expect("Missing the GITHUB_CLIENT_ID environment variable."));
  ...
  if let Ok(token) = token_res {
    ...
    let scopes = if let Some(scopes_vec) = token.scopes() {
      scopes_vec.iter().flat_map(|comma_separated| comma_separated.split(',')).collect::<Vec<_>>()
    } else {
      Vec::new()
    };
