...
use fusions::ai::graph_flow::GraphError::TaskExecutionFailed;
...
fn get_chat_model() -> anyhow::Result<CompletionModel> {
  ...
  let client = fusions::ai::providers::openai_compatible::Client::builder(&api_key)
    ...
    .build();
  Ok(client.completion_model("deepseek-flash"))
}
...
async fn chat_once(model: &CompletionModel, prompt: &str, history: Vec<core_types::Message>) -> anyhow::Result<String> {
  ...
  let request = CompletionRequest::from_history(model.model(), None, full_history, vec![], None, None, None, None)
...
// Note: fastembed 5.x API differs from 0.4, this example uses a stub for now.
// To use fastembed, uncomment and adapt to the new API:
// async fn embed_query(text: &str) -> anyhow::Result<Vec<f32>> {
//   let input = text.to_owned();
//   info!("Generating embedding for text: {}", text);
//   // Off-load the potentially expensive ONNX inference to a blocking thread
...
// Stub implementation for now - returns a dummy embedding
async fn embed_query(text: &str) -> anyhow::Result<Vec<f32>> {
  info!("Generating stub embedding for text: {}", text);
  // Return a dummy 384-dimensional embedding (same as AllMiniLML6V2)
  Ok(vec![0.1; 384])
}
...
impl Task for QueryRefinementTask {
  async fn run(&self, context: Context) -> fusions::ai::graph_flow::Result<TaskResult> {
